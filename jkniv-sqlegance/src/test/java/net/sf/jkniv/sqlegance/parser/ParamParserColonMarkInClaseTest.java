...
public class ParamParserColonMarkInClaseTest
{
...
//    String sqlExpected = "select id, name from Roles where id = ?   and name = ?            and dt = to_date(?  ,'YYYY-MM-DD HH24:MI:SS')";
    ...
    @Test
    public void whenParserParamsUseColonMarkUsingSimpleINclause()
    {
        String[] params = {"A","B","C"};
        String sql                 = "select * from Roles where name IN (:in:names)";
        String sqlExpected         = "select * from Roles where name IN (?,?,?)";
        ParamParser parser = ParamParserFactory.getInstance(ParamMarkType.COLON);//new ParamParserColonMark();
    ...
    @Test
    public void whenParserParamsUseColonMarkUsingDoubleINclause()
    {
        String[] params = {"A","B","C"};
    ...
    @Test
    public void whenParserParamsUseColonMarkINclauseWithQuestion()
    {
        String[] params = {"A","B","C"};
