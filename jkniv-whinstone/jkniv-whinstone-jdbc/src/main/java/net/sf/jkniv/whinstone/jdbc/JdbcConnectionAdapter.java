...
public class JdbcConnectionAdapter implements ConnectionAdapter
{
    ...
    private <T, R> StatementAdapter<T, R> newStatement(Queryable queryable)
    {
        ...
        StatementAdapter<T, R> adapter = new JdbcPreparedStatementAdapter(stmt, queryable);
        return adapter;
    }
    ...
    private <T, R> StatementAdapter<T, R> newInsertStatement(Queryable queryable)
    {
        ...
        if (isql.isAutoGenerateKey() && isql.getAutoGeneratedKey().isAutoStrategy())
        {
            ...
            auto = new JdbcAutoGeneratedKey(stmt, handlerException);
        }
        else
        {
...
//    }
    ...
    public <T, R> Command asUpdateCommand(Queryable queryable)
    {
        ...
        if (queryable.isTypeOfBulk())
            command = new BulkJdbcCommand(queryable, this.conn)
                .with(this.newStatement(queryable));
        else
            command = new DefaultJdbcCommand(queryable, this.conn)
                            .with(this.newStatement(queryable));
    ...
    public <T, R> Command asAddCommand(Queryable queryable)
    {
        ...
        if (sql.isAutoGenerateKey())
        {
            ...
            {
                StatementAdapter stmt = this.newInsertStatement(queryable);
                command = new AddAutoKeyJdbcCommand(queryable, this.conn).with(stmt);
            }
            else if (sql.getAutoGeneratedKey().isSequenceStrategy())
            {
                command = new AddSequenceKeyJdbcCommand(queryable, this.conn)
                            .with(this.newInsertStatement(queryable));
            }
        }
    ...
    public <T, R> Command asSelectCommand(Queryable queryable, ResultRow<T, R> overloadResultRow)
    {
        ...
        command = new DefaultJdbcQuery(queryable, this.conn).with(stmt);
        return command;
    }
