...
import net.sf.jkniv.sqlegance.builder.xml.AutoGeneratedKeyTag;
...
class XmlStatement
{
    ...
    public Map<String, Sql> build(SqlDialect sqlDialect)
    {
        ...
        processTagsElements(ROOT_NODE + "/" + SelectTag.TAG_NAME, "", statements, sqlDialect);
        processTagsElements(ROOT_NODE + "/" + InsertTag.TAG_NAME, "", statements, sqlDialect);
        processTagsElements(ROOT_NODE + "/" + UpdateTag.TAG_NAME, "", statements, sqlDialect);
        processTagsElements(ROOT_NODE + "/" + DeleteTag.TAG_NAME, "", statements, sqlDialect);
        ...
        if (nodes != null)
        {
            for (int i = 0; i < nodes.getLength(); i++)
            {
                ...
                if (node.getNodeType() == Node.ELEMENT_NODE)
                {
                    ...
                    processTagsElements(NODE_PACKAGE_BYNAME + SelectTag.TAG_NAME, name, statements, sqlDialect);
                    processTagsElements(NODE_PACKAGE_BYNAME + InsertTag.TAG_NAME, name, statements, sqlDialect);
                    processTagsElements(NODE_PACKAGE_BYNAME + UpdateTag.TAG_NAME, name, statements, sqlDialect);
                    processTagsElements(NODE_PACKAGE_BYNAME + DeleteTag.TAG_NAME, name, statements, sqlDialect);
    ...
    private SqlTag buildSql(Element element, String nodeName, String xpathSql)
    {
        ...
        else
        {
            ...
            for (int j = 0; j < element.getChildNodes().getLength(); j++)
            {
                ...
                if (tag.isInsertable() && textTag instanceof AutoGeneratedKeyTag)
                {
                    AutoGeneratedKeyTag autoGeneratedKeyTag = (AutoGeneratedKeyTag)textTag;
                    InsertTag insertTag = (InsertTag) tag;
                    insertTag.setAutoGeneratedKeyTag(autoGeneratedKeyTag);
                }
    ...
    private AutoGeneratedKeyTag processAutoGenerateKeyTag(Element element)
    {
        String type = element.getAttribute(AutoGeneratedKeyTag.ATTRIBUTE_STRATEGY);
        String columns = element.getAttribute(AutoGeneratedKeyTag.ATTRIBUTE_COLUMNS);
        String props = element.getAttribute(AutoGeneratedKeyTag.ATTRIBUTE_PROPS);
        String text = element.getTextContent();
        AutoGeneratedKeyTag tag = new AutoGeneratedKeyTag(type, columns, props, text);
        return tag;
    }
            ...
            String groupBy, ValidateType validateType, Element element)
    {
        ...
        {
            tag = new SelectTag(id, type, isolation, timeout, batch, cache, hint, resultSetType, resultSetConcurrency,
            ...
            for (int i = 0; i < oneToManyList.getLength(); i++)
            {
                ...
                ((SelectTag)tag).addOneToMany(tagOtM);
            }
        }
        else if (InsertTag.TAG_NAME.equals(nodeName))
        {
            tag = new InsertTag(id, type, isolation, timeout, batch, hint, validateType);
        }
        else if (UpdateTag.TAG_NAME.equals(nodeName))
        {
            tag = new UpdateTag(id, type, isolation, timeout, batch, hint, validateType);
        }
        else if (DeleteTag.TAG_NAME.equals(nodeName))
        {
            tag = new DeleteTag(id, type, isolation, timeout, batch, hint, validateType);
        }
    ...
    private ITextTag getDynamicNode(Node node)
    {
        ...
        else if (AutoGeneratedKeyTag.TAG_NAME.equals(nodeName))
        {
            Element element = (Element) node;
